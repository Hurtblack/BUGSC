...
class AgentPromptBuilder(private val profile: AgentProfile) {
    ...
    fun buildToolCalling(
        ...
        val system = buildString {
            ...
            appendLine("- 用户询问本地资料没有覆盖的飞船、挂点、导弹架、导弹或组件详情时，优先调用 search_online_ship；已有飞船 ID 时可直接调用 get_online_ship_detail，需要挂点关联物品时再调用 get_online_component_detail。")
            ...
            appendLine("- 市场续页意图：search_market 结果提示还有未显示订单，用户追问“继续”“下一页”“还有吗”“多给点”时，复用最近一次 query/side，并把 page 加 1；不要重复请求第 1 页。")
